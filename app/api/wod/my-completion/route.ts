...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data: completion, error } = await supabase
      ...
      .select("id, user_id, original_wod_post_id, completion_post_id, completion_type, completion_time_seconds, rounds_completed, total_reps, weight_kg, score, rx, notes, completed_at")
      .eq("user_id", user.id)
