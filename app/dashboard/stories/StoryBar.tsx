...
interface OverlayElement {
    id: string
    type: 'text' | 'image' | 'sticker' | 'workout_sticker' | 'pr_sticker' | 'attribution'
    content: string
    x: number
    y: number
    scale: number
                            ...
                            return (
                                <div className="relative group">
                                    <div className="bg-black/80 backdrop-blur-2xl border border-white/10 px-4 py-2.5 rounded-full shadow-2xl flex items-center gap-3">
                                        <div className="w-8 h-8 rounded-full border border-brand-red/30 overflow-hidden relative flex-shrink-0">
                                            {attr.avatar ? (
                                                <Image src={attr.avatar} alt={attr.username} fill className="object-cover" />
                                            ) : (
                                                <div className="w-full h-full bg-brand-red/10 flex items-center justify-center text-[8px] font-black text-brand-red">RF</div>
                                            )}
                                        </div>
                                        <div className="flex flex-col pr-2">
                                            <span className="text-[7px] text-brand-red font-black uppercase tracking-[0.2em] leading-none mb-0.5">COMPARTIDO DE</span>
                                            <span className="text-white font-black italic uppercase text-xs tracking-tighter leading-none">@{attr.username}</span>
                                        </div>
                                    </div>
                                    {previewUrl && (
                                        <button
                                            ...
                                            {overlay.type === 'text' ? (
                                                ...
                                                (() => {
                                                    try {
                                                        const attr = JSON.parse(overlay.content);
                                                        return (
                                                            <div className="bg-black/80 backdrop-blur-2xl border border-white/10 px-4 py-2.5 rounded-full shadow-2xl flex items-center gap-3 active:scale-95 transition-transform">
                                                                <div className="w-8 h-8 rounded-full border border-brand-red/30 overflow-hidden relative flex-shrink-0">
                                                                    {attr.avatar ? (
                                                                        <Image src={attr.avatar} alt={attr.username} fill className="object-cover" />
                                                                    ) : (
                                                                        <div className="w-full h-full bg-brand-red/10 flex items-center justify-center text-[10px] font-black text-brand-red">RF</div>
                                                                    )}
                                                                </div>
                                                                <div className="flex flex-col pr-2">
                                                                    <span className="text-[7px] text-brand-red font-black uppercase tracking-[0.2em] leading-none mb-0.5">COMPARTIDO DE</span>
