        ...
        <div className="max-w-5xl mx-auto space-y-12 animate-fade-in pb-20">
            ...
            <div className="relative group rounded-[40px] overflow-hidden border border-white/5 shadow-2xl dark-section">
                ...
                <div className="absolute bottom-0 left-0 right-0 p-2 md:p-10 flex flex-col md:flex-row md:items-end justify-between gap-4 md:gap-8 z-20">
                    <div className="flex items-end gap-0 md:gap-6 relative">
                        <div className="w-20 h-20 md:w-40 md:h-40 rounded-full border-2 md:border-8 border-black bg-brand-gray overflow-hidden relative shadow-2xl shrink-0 transform translate-y-8 md:translate-y-0 -left-1 md:left-0 z-10">
                            <Image
                        ...
                        <div className="pb-1 md:pb-4 min-w-0 transform translate-y-6 md:translate-y-0 z-20 ml-2 md:ml-0 w-auto mr-auto">
                            <div className="flex items-center gap-1.5 md:gap-3 -ml-5 md:ml-0">
                                <h1 className="text-xl md:text-4xl font-heading font-black text-white italic uppercase tracking-tighter truncate leading-none shadow-black drop-shadow-md">{profile.full_name}</h1>
                                {profile.level >= 5 && <Star className="w-4 h-4 md:w-6 md:h-6 text-yellow-500 fill-yellow-500 shrink-0" />}
                            </div>
                            <p className="text-brand-red font-black tracking-[0.3em] text-[10px] md:text-sm uppercase mt-0.5 truncate shadow-black drop-shadow-sm -ml-5 md:ml-0">@{profile.username}</p>

                            {/* Mobile Stats Summary */}
                            <div className="block md:hidden mt-2 space-y-2 w-max max-w-[180px] relative z-0">
                                {/* Mini Combat Record */}
                                <div className="flex items-center gap-3 bg-black/60 rounded-lg p-1.5 border border-white/5 backdrop-blur-md">
                                    <div className="flex-1 text-center">
                                        <div className="text-white font-black italic text-sm">{combatStats.wins}</div>
                                        <div className="text-[6px] text-green-500 font-black uppercase tracking-wider">W</div>
                                    </div>
                                    <div className="w-px h-6 bg-white/10"></div>
                                    <div className="flex-1 text-center">
                                        <div className="text-white font-black italic text-sm">{combatStats.losses}</div>
                                        <div className="text-[6px] text-brand-red font-black uppercase tracking-wider">L</div>
                                    </div>
                                    <div className="w-px h-6 bg-white/10"></div>
                                    <div className="bg-brand-red/10 px-1.5 py-0.5 rounded text-[7px] font-black text-brand-red">
                                        {combatStats.total > 0 ? ((combatStats.wins / combatStats.total) * 100).toFixed(0) : 0}%
                                    </div>
                                </div>
            ...
            <div className="grid lg:grid-cols-12 gap-10">
                ...
                <div className="lg:col-span-8 space-y-10">
                    ...
                    {!canViewContent ? (
                        ...
                        <>
                            {/* Media Gallery (Mobile & Desktop Top Placement) */}
                            <UserMediaGallery userId={profile.id} />

                            {/* Bio Card */}
                            <div className="hidden md:block bg-brand-gray/30 border border-white/5 p-8 rounded-[40px] backdrop-blur-xl">
                ...
                <div className="lg:col-span-4 space-y-8">
                    ...
                    {canViewContent ? (
                        <>
                            {/* Combat Record Card */}
                            <div className="hidden md:block bg-black/60 border border-brand-red/20 rounded-[40px] p-8 relative overflow-hidden group">
                            ...
                            {/* Vitals Card */}
                            <div className="hidden md:block bg-brand-gray/30 border border-white/10 rounded-[40px] p-8 backdrop-blur-md">
