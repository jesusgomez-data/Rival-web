    ...
    const handleUpgrade = async (plan: any) => {
        ...
        try {
            if (plan.id === 'free') {
                ...
                const priceId = plan.id === 'premium'
                    ? process.env.NEXT_PUBLIC_STRIPE_PRICE_PREMIUM
                    : process.env.NEXT_PUBLIC_STRIPE_PRICE_ELITE;

                if (!priceId || priceId.includes('price_...')) {
                    // FALLBACK FOR DEVELOPMENT (Direct update if keys not set)
                    console.log("Stripe keys not set, using mock update");
                    const supabase = createClient();
                    await supabase.from('profiles').update({ subscription_tier: plan.id }).eq('id', profile?.id);
                    setProfile({ ...profile, subscription_tier: plan.id });
                    alert(`[MOCK] Te has unido al plan ${plan.name}`);
                } else {
                ...
                {plans.map((plan) => {
                    ...
                    return (
                        ...
                        >
                            ...
                            <button
                                onClick={() => isCurrent ? createPortalSession() : handleUpgrade(plan)}
