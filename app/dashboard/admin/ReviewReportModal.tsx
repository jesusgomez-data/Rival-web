...
import { X, CheckCircle, AlertTriangle, User, MessageSquare, Trash2, Shield } from 'lucide-react';
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <div className="relative bg-white dark:bg-[#0a0a0a] border border-gray-200 dark:border-white/10 w-full max-w-lg rounded-3xl overflow-hidden shadow-2xl animate-in zoom-in-95 duration-200 transition-colors">
                ...
                <div className="p-6 space-y-6">
                    <div className="bg-gray-100 dark:bg-white/5 rounded-2xl p-4 border border-gray-200 dark:border-white/5 transition-colors">
                        <div className="flex items-center gap-3 mb-4">
                            <div className="w-10 h-10 rounded-full bg-brand-red/20 flex items-center justify-center text-brand-red">
                                <User className="w-5 h-5" />
                            </div>
                            <div>
                                <div className="text-black dark:text-white font-bold">@{report.user}</div>
                        ...
                        <div className="space-y-4">
                            <div>
                                ...
                                <div className="text-sm text-black dark:text-white bg-white dark:bg-white/5 p-3 rounded-xl border border-gray-200 dark:border-white/5 shadow-sm dark:shadow-none font-medium">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <div className="text-xs text-black dark:text-white font-mono truncate">{report.target}</div>
                                </div>
                                <div>
                                    <label className="text-[10px] font-black uppercase tracking-widest text-gray-500 block mb-1">Fecha</label>
                                    <div className="text-xs text-black dark:text-white font-mono">{report.date}</div>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 gap-3">
                        {report.status !== 'Resolved' && (
                            <>
                                ...
                                <button
                                    ...
                                    disabled={!!loading}
                                    className="w-full bg-red-900/10 dark:bg-red-900/20 hover:bg-red-900 text-red-600 dark:text-red-400 hover:text-white font-bold py-3 rounded-xl transition-all text-xs uppercase tracking-wider flex items-center justify-center gap-2 border border-red-900/20 dark:border-red-900/30"
                                >
                                ...
                                <button
                                    ...
                                    disabled={!!loading}
                                    className="w-full bg-gray-100 dark:bg-white/5 hover:bg-gray-200 dark:hover:bg-white/10 text-gray-600 dark:text-white font-bold py-3 rounded-xl transition-all text-xs uppercase tracking-wider flex items-center justify-center gap-2 border border-gray-200 dark:border-white/5"
                                >
                        ...
                        <button
                            onClick={onClose}
                            className="w-full bg-gray-100 dark:bg-white/5 hover:bg-gray-200 dark:hover:bg-white/10 text-gray-500 dark:text-gray-400 font-bold py-3 rounded-xl transition-all text-xs uppercase tracking-wider"
                        >
                            Cerrar
                        </button>
                    </div>
