...
export default function CreatePost({ currentUser }: { currentUser: any }) {
    ...
    return (
        <div className="bg-brand-gray/30 border border-white/10 rounded-[32px] p-6 backdrop-blur-md mb-8 relative z-10">
            {/* Post Type Selector */}
            <div className="flex gap-2 mb-6">
                <button
                    onClick={() => setPostType('standard')}
                    className={`flex-1 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all flex items-center justify-center gap-2 border ${postType === 'standard' ? 'bg-white/10 border-white/20 text-white' : 'border-transparent text-gray-500 hover:text-gray-300'}`}
                >
                ...
                <button
                    ...
                    className={`flex-1 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all flex items-center justify-center gap-2 border ${postType === 'pr' ? 'bg-brand-red/10 border-brand-red/30 text-brand-red' : 'border-transparent text-gray-500 hover:text-gray-300'}`}
                >
                    ...
                    <form onSubmit={handlePost}>
                        {postType === 'standard' ? (
                            <textarea
                                ...
                                className="w-full bg-transparent text-white placeholder:text-gray-500 text-sm md:text-lg resize-none focus:outline-none mb-4 min-h-[50px] md:min-h-[60px]"
                                rows={2}
                            />
                        ) : (
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                                <div className="space-y-1">
                                    ...
                                    <input
                                        ...
                                        className="w-full bg-black/40 border border-white/10 rounded-xl px-4 py-2 text-white focus:outline-none focus:border-brand-red/50 text-sm"
                                    />
                                </div>
                                <div className="space-y-1">
                                    ...
                                    <input
                                        ...
                                        className="w-full bg-black/40 border border-white/10 rounded-xl px-4 py-2 text-brand-red font-black focus:outline-none focus:border-brand-red/50 text-sm"
                                    />
                                </div>
                        ...
                        {preview && (
                            <div className="relative mb-4 rounded-xl overflow-hidden border border-white/10 bg-black/40 w-fit max-w-full">
                                ...
                                <button
                                    ...
                                    className="absolute top-2 right-2 bg-black/60 text-white p-1 rounded-full hover:bg-brand-red transition-colors"
                                >
                                    <X className="w-4 h-4" />
                                </button>
                            </div>
                        ...
                        <div className="flex items-center justify-between border-t border-white/5 pt-4">
                            <div className="flex gap-2 relative">
                                <button
                                    ...
                                    className="p-2 text-brand-red hover:bg-brand-red/10 rounded-xl transition-colors flex items-center gap-2 relative z-10"
                                    title="Add Media"
                                >
                                ...
                                {postType === 'standard' && (
                                    <div ref={emojiPickerRef} className="relative z-20">
                                        <button
                                            ...
                                            className={`p-2 rounded-xl transition-colors flex items-center gap-2 ${showEmojiPicker ? 'text-yellow-400 bg-yellow-400/10' : 'text-gray-400 hover:text-yellow-400 hover:bg-yellow-400/10'}`}
                                            title="Add Emoji"
                                        >
                            ...
                            <button
                                ...
                                className="bg-brand-red text-white px-6 py-2 rounded-xl text-xs font-black uppercase tracking-widest hover:bg-red-600 transition-all disabled:opacity-50 flex items-center gap-2"
                            >
                                {isPosting ? (
