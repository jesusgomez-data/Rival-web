...
export default function TrainerPublicProfile() {
    ...
    async function loadProfile() {
        ...
        const [plansRes, reviewsRes, followRes, memberRes, requestsRes, servicesRes, availRes, svcReviewsRes] = await Promise.all([
            ...
            user ? supabase.from('members').select('id, status, cancellation_requested_at').eq('center_id', id).eq('user_id', user.id).in('status', ['active', 'trial']).maybeSingle() : Promise.resolve({ data: null }),
    ...
    return (
        <div className="min-h-screen bg-black text-white pb-10 flex flex-col items-center px-0 sm:px-4">
            ...
            <div className="px-4 sm:px-8 py-4 flex flex-wrap gap-3 items-center">
                ...
                {memberRecord && ['active', 'trial'].includes(memberRecord.status) && currentUser?.id !== trainer.owner_id && (
                    <button
                        ...
                        disabled={!!memberRecord.cancellation_requested_at}
                        className={`flex items-center gap-1.5 px-4 py-2.5 rounded-xl font-bold text-sm transition-all shadow-lg ${
                            memberRecord.cancellation_requested_at
                        ...
                        title={memberRecord.cancellation_requested_at ? "Baja en trámite" : "Solicitar baja como alumno"}
                    >
                        <X className="w-4 h-4" />
                        {memberRecord.cancellation_requested_at ? "Baja Solicitada" : "Solicitar Baja"}
                    </button>
                )}
            </div>
            ...
            {/* Cancellation Modal */}
            {showCancellationModal && memberRecord && (
                <CancellationRequestModal
                    orgId={trainer.id}
                    orgName={trainer.name}
                    orgType="professional"
                    onClose={() => setShowCancellationModal(false)}
                    onSuccess={() => {
                        setShowCancellationModal(false)
                        setMemberRecord((prev: any) => prev ? { ...prev, cancellation_requested_at: new Date().toISOString() } : null)
                    }}
                />
            )}
        </div>
