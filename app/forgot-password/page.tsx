            ...
            <div className="min-h-screen flex items-center justify-center p-8 bg-background text-foreground transition-colors duration-300">
                <div className="max-w-md w-full text-center">
                    <div className="mb-8">
                        ...
                        <h1 className="text-3xl font-heading font-bold mb-4">{t.login.checkEmailTitle}</h1>
                        <p className="text-muted-foreground text-lg leading-relaxed">
                            {t.login.checkEmailMessage}
                        </p>
                    </div>
                    <Link
                        href="/login"
        ...
        <div className="min-h-screen flex font-sans selection:bg-brand-red selection:text-white transition-colors duration-300 bg-background text-foreground">
            ...
            <div className="hidden lg:block w-1/2 relative overflow-hidden">
                ...
                <div className="absolute bottom-12 left-12 z-10">
                    ...
                    <p className="text-muted-foreground max-w-md text-lg">{t.login.forgotPasswordDesc}</p>
                </div>
            </div>
            ...
            <div className="w-full lg:w-1/2 flex items-center justify-center p-8 pt-24 lg:pt-8 relative min-h-screen lg:min-h-0">
                ...
                <div className="max-w-md w-full">
                    <div className="text-center mb-10">
                        ...
                        <h1 className="text-3xl font-heading font-bold mb-2 text-foreground">{t.login.forgotPasswordTitle}</h1>
                        <p className="text-muted-foreground">{t.login.forgotPasswordDesc}</p>
                    </div>
                    ...
                    <form className="space-y-6" action={formAction}>
                        ...
                        >
                            {isPending ? t.login.authenticating : t.login.sendResetLink} <ArrowRight className="w-5 h-5 group-hover:translate-x-1 transition-transform" />
                        </button>
                    </form>

                    <div className="mt-8 text-center">
                        <p className="text-muted-foreground">{t.signup.alreadyHaveAccount} <Link href="/login" className="font-bold hover:underline text-foreground">{t.login.submit}</Link></p>
                    </div>
                </div>
            </div>
        </div>
